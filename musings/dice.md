...
* __Trusted Computing Base (TCB)__ : All the security relevant HW and SW in a DICE device.  DICE organizes the TCB in multiple "components", which are grouped in "layers" stacked one on top of the other, starting with the DICE engine.  A layer boundary represents an explicit handover point from one component to one or more components in the "next" stage of the boot chain.
...
* __TCB Info__ - The information model of a TCB component.  It gathers a standard set of attributes (and associate types) that describe a TCB, for example: its model, vendor, Firmware digests, version, etc.
...
Even though further extensions have been defined (for the full list see Section 6.1 of ["Attestation Architecture"](https://trustedcomputinggroup.org/resource/dice-attestation-architecture/) -- for example one that allows unsigned [EAT](https://datatracker.ietf.org/doc/draft-ietf-rats-eat/) claims to be embedded in an X.509 extension -- `DiceTcbInfo` is the (currently) preferred way.
...
We expect that, typically, DICE evidence submitted for verification is not bound to an explicit challenge-response session.  This is likely to require defining a new Veraison API alongside the existing [challenge-response](api/challenge-response/README.md) endpoint.
...
Veraison [DICE implementation](https://github.com/veraison/dice) will need to be extended with APIs for claims extraction.  The existing certificate chain verification API should be OK as-is.

### Endorsements Provisioning

By definition, native CoRIM fully supports TCG DICE, so there should be no extra work needed to profile the [corim](https://github.com/veraison/corim) API, which already implements the entirety of CoRIM.
